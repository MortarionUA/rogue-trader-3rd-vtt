...
export class DarkHeresyShipWeaponSheet extends DarkHeresyItemContainerSheet {
    ...
    get template() {
        return `systems/rogue-trader-3rd/templates/item/item-ship-weapon-sheet.hbs`;
    }
    ...
    canAdd(itemData) {
        ...
        if (this.item.items.some((i) => i.name === itemData.name)) {
            ui.notifications.info('Ship Weapon can only hold one ' + itemData.name);
            return false;
        }
